...
describe('CorsPlugin', () => {
    it.each(['https://some-external.js', 'http://10.1.1.25/assets/app.js'])('should replace JS urls', (jsUrl) => {
    ...
    it.each([
        ...
        `body { background-image: url('http://10.1.1.25/images/logo.png'); }`,
    ])('should replace asset urls in stylesheets', (content: string) => {
        expect(CorsPlugin._replaceAssetCssUrls(content)).toMatchSnapshot()
    })

    it.each([
        'https://app.posthog.com/fonts/my-font.woff2?t=1234',
        'https://app.posthog.com/fonts/my-font.ttf',
        'http://10.1.1.25/static/logo.png',
        'https://app.posthog.com/static/styles.css',
    ])('should replace asset urls in links', (content: string) => {
    ...
    it('can replace srcset urls', () => {
        expect(CorsPlugin._replaceSrcSetUrls('http://10.1.1.25/a.png 1x, https://app.posthog.com/b.png 2x')).toEqual(
            'https://replay.ph-proxy.com/proxy?url=http://10.1.1.25/a.png 1x, https://replay.ph-proxy.com/proxy?url=https://app.posthog.com/b.png 2x'
        )
    })
    ...
    it('can replace image src and srcset attributes', () => {
        ...
        expect(el.srcset).toEqual(
            'https://replay.ph-proxy.com/proxy?url=http://10.1.1.25/static/logo.png 1x, https://replay.ph-proxy.com/proxy?url=https://app.posthog.com/static/logo@2x.png 2x'
        )
    })
})
