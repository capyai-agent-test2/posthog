...
def add_geoip_template(apps, schema_editor):
    HogFunctionTemplate = apps.get_model("posthog", "HogFunctionTemplate")
    HogFunction = apps.get_model("posthog", "HogFunction")

    template, _ = HogFunctionTemplate.objects.update_or_create(
        template_id=GEOIP_TEMPLATE["template_id"],
        defaults=GEOIP_TEMPLATE,
    )

    legacy_geoip_functions = HogFunction.objects.filter(
        Q(template_id__isnull=True) | Q(template_id=""),
        type="transformation",
        name="GeoIP",
    ).filter(Q(icon_url="/static/transformations/geoip.png") | Q(description__icontains="geoip"))

    legacy_geoip_functions.update(template_id=template.template_id, hog_function_template=template)
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(add_geoip_template, reverse_code=migrations.RunPython.noop),
    ]
