...
class TestExternalDataSource(APIBaseTest):
    ...
    @patch(
        ...
        side_effect=Exception("Source cleanup failed"),
    )
    def test_delete_external_data_source_soft_deletes_even_if_source_registry_cleanup_fails(
        self, _mock_get_source, mock_capture_exception
    ):
        source = self._create_external_data_source()
        schema = self._create_external_data_schema(source.pk)
