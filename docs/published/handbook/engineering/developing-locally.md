...
**Port conflicts**
If you see a port binding error for 5432, you have Postgres running locally. Use `lsof -i :5432` to find the process, then `sudo service postgresql stop` to stop it. You may also see errors like `role "posthog" does not exist` or `password authentication failed for user "posthog"`, which can indicate that a local PostgreSQL instance is being used instead of the expected containerized one.
